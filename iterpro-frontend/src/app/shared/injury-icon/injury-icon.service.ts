...
import { AvailabiltyService, getMomentFormatFromStorage } from '@iterpro/shared/utils/common-utils';
...
export class InjuryIconService {
	...
	public parsePlayer(injuries: Injury[], period: Date): {tooltip: string; icon: string} {
		const status = this.getHealthStatus(injuries, period);
		const tooltip = this.getInjuryTooltip(injuries, status, period);
		const icon = healthIcons[status];
		return { tooltip, icon };
	}

	public getHealthStatusLabel(injuries: Injury[], period: Date): string {
		return this.getHealthStatus(injuries, period);
	}

	private getInjuryTooltip(injuries: Injury[] = [], status: HealthStatus, period: Date): string {
	...
	public getHealthStatus(injuries: Injury[], period?: Date): HealthStatus {
		...
		return this.getInjuryStatus(filtered, period);
	}
	...
	private translateStatus(status: HealthStatus): string {
		switch (status) {
			case 'notAvailable': {
