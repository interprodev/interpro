...
import { DeviceMetricDescriptor, IBaseEtlPlayerService, PlayerStat, Threshold } from '@iterpro/shared/data-access/sdk';

@Injectable({
	providedIn: 'root'
})
export class BaseEtlPlayerThirdPartyService implements IBaseEtlPlayerService {
	...
	constructor() {
		...
		this.metricsMapping.push(
			new DeviceMetricDescriptor({
				algo: false,
				metricLabel: 'Matches In Start',
				metricName: 'matchesInStart',
				defaultValue: 0
			})
		);
		...
		this.metricsMapping.push(
			new DeviceMetricDescriptor({
				algo: false,
				metricLabel: 'Matches Substituted',
				metricName: 'matchesSubstituted',
				defaultValue: 0
			})
		);
		...
		this.metricsMapping.push(
			new DeviceMetricDescriptor({
				algo: false,
				metricLabel: 'Matches ComingOff',
				metricName: 'matchesComingOff',
				defaultValue: 0
			})
		);
		...
		this.metricsMapping.push(
			new DeviceMetricDescriptor({
				algo: false,
				metricLabel: 'Minutes On Field',
				metricName: 'minutesOnField',
				defaultValue: 0
			})
		);
		...
		this.metricsMapping.push(
			new DeviceMetricDescriptor({
				algo: false,
				metricLabel: 'Minutes Tagged',
				metricName: 'minutesTagged',
				defaultValue: 0
			})
		);
		...
		this.metricsMapping.push(
			new DeviceMetricDescriptor({
				algo: false,
				metricLabel: 'Field Aerial DuelsWon',
	...
	getPlayerStatsFromCsv(): PlayerStat[] | null {
		return null;
	}
	...
	getDurationField(): DeviceMetricDescriptor {
		return this.metricsMapping.find(
			(val: DeviceMetricDescriptor) => val.metricName === 'minutesOnField'
		) as DeviceMetricDescriptor;
	}
}
